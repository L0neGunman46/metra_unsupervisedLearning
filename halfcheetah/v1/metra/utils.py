...
#convert numpyarray to pytorch tensor
def _to_t(x, device):
...
def evaluate_and_visualize(agent, config, training_step):
    ...
    colors = cm.get_cmap('hsv', num_skills)(np.linspace(0,1,num_skills))
    ...
    for i in range(num_skills):
        ...
        max_steps = getattr(eval_env, "_max_episode_steps", None)
        if max_steps is None and eval_env.spec is not None:
    ...
    data_filename = f'results/trajectories/trajectories_seed{seed}_step{training_step}.json'
    with open(data_filename, 'w') as f:
        json.dump(trajectories_data, f, indent=2)
